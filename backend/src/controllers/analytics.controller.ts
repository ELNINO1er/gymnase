  ...
  try {
    ...
    const hourly = await query<any[]>(
      `SELECT HOUR(check_in_time) as hour, COUNT(*) as entries,
       ...
       WHERE gym_id = ? AND status = 'VALID' AND check_in_time >= DATE_SUB(NOW(), INTERVAL ? DAY)
    ...
    const sessions = await query<any[]>(
      `SELECT s.name, s.capacity, COUNT(r.id) as total_bookings,
       ...
       LEFT JOIN reservations r ON r.session_id = s.id
         AND r.gym_id = s.gym_id
         AND r.reservation_date >= DATE_SUB(CURDATE(), INTERVAL ? DAY)
       WHERE s.gym_id = ? AND s.is_active = TRUE
  ...
  try {
    ...
    const byDayOfWeek = await query<any[]>(
      `SELECT DAYNAME(paid_at) as day_name, DAYOFWEEK(paid_at) as day_num,
       ...
       FROM payments
       WHERE gym_id = ? AND status = 'PAID' AND paid_at >= DATE_SUB(NOW(), INTERVAL ? DAY)
  ...
  try {
    ...
    const members = await query<any[]>(
      `SELECT u.id, u.full_name, u.member_code,
       ...
       FROM users u
       LEFT JOIN attendance_logs a ON a.user_id = u.id AND a.gym_id = u.gym_id AND a.status = 'VALID' AND a.check_in_time >= DATE_SUB(NOW(), INTERVAL ? DAY)
       LEFT JOIN reservations r ON r.user_id = u.id AND r.gym_id = u.gym_id AND r.reservation_date >= DATE_SUB(CURDATE(), INTERVAL ? DAY) AND r.status NOT IN ('CANCELLED')
       LEFT JOIN payments p ON p.user_id = u.id AND p.gym_id = u.gym_id AND p.paid_at >= DATE_SUB(NOW(), INTERVAL ? DAY)
       WHERE u.gym_id = ? AND u.role = 'MEMBER' AND u.status = 'ACTIVE'
       GROUP BY u.id, u.full_name, u.member_code
       ORDER BY visits DESC
       LIMIT ${Number(limitN)}`,
      [days, days, days, gymId]
    );
...
export async function getRetention(req: Request, res: Response) {
  try {
    ...
    const [totalMembers] = await query<any[]>("SELECT COUNT(*) as c FROM users WHERE gym_id = ? AND role = 'MEMBER' AND status != 'DELETED'", [gymId]);
    const [active] = await query<any[]>("SELECT COUNT(*) as c FROM users WHERE gym_id = ? AND role = 'MEMBER' AND status = 'ACTIVE'", [gymId]);
    const [expired] = await query<any[]>("SELECT COUNT(*) as c FROM users WHERE gym_id = ? AND role = 'MEMBER' AND status = 'EXPIRED'", [gymId]);
    const [suspended] = await query<any[]>("SELECT COUNT(*) as c FROM users WHERE gym_id = ? AND role = 'MEMBER' AND status = 'SUSPENDED'", [gymId]);
    ...
    const [renewed] = await query<any[]>(
      `SELECT COUNT(*) as c FROM subscriptions
       WHERE gym_id = ? AND status = 'ACTIVE' AND MONTH(created_at) = MONTH(CURDATE()) AND YEAR(created_at) = YEAR(CURDATE())
         AND user_id IN (SELECT user_id FROM subscriptions WHERE gym_id = ? AND status = 'EXPIRED')`,
      [gymId, gymId]
    );
    const [expiredThisMonth] = await query<any[]>(
      `SELECT COUNT(*) as c FROM subscriptions
       WHERE gym_id = ? AND status = 'EXPIRED' AND MONTH(created_at) = MONTH(CURDATE()) AND YEAR(created_at) = YEAR(CURDATE())`,
      [gymId]
    );
