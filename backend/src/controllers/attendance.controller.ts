  ...
  try {
    ...
    const members = await query<any[]>(
      `SELECT a.id, a.user_id, a.check_in_time, a.method,
       ...
       WHERE a.gym_id = ? AND a.check_out_time IS NULL AND a.status = 'VALID' AND DATE(a.check_in_time) = CURDATE()
...
export async function manualCheckIn(req: Request, res: Response) {
  try {
    ...
    const [user] = await query<any[]>("SELECT id, full_name, status FROM users WHERE id = ? AND gym_id = ?", [user_id, gymId]);
    if (!user) {
    ...
    await query<any>(
      `INSERT INTO attendance_logs (gym_id, user_id, method, status, reason) VALUES (?, ?, 'MANUAL', ?, ?)`,
      [gymId, user_id, status, reason]
    );
  ...
  try {
    ...
    const result = await query<any>(
      `UPDATE attendance_logs SET check_out_time = NOW()
       WHERE gym_id = ? AND user_id = ? AND check_out_time IS NULL AND status = 'VALID' AND DATE(check_in_time) = CURDATE()
  ...
  try {
    ...
    const logs = await query<any[]>(
      `SELECT a.*, u.full_name, u.member_code
       FROM attendance_logs a
       JOIN users u ON u.id = a.user_id
       WHERE a.gym_id = ? AND DATE(a.check_in_time) = CURDATE()
       ORDER BY a.check_in_time DESC`,
      [gymId]
    );
  ...
  try {
    ...
    const daily = await query<any[]>(
      `SELECT DATE(check_in_time) as day, COUNT(*) as entries
       ...
       WHERE gym_id = ? AND status = 'VALID' AND check_in_time >= DATE_SUB(CURDATE(), INTERVAL ? DAY)
